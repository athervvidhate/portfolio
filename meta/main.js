...
function processCommits() {
  commits = d3
    ...
    .map(([commit, lines]) => {
      ...
      let ret = {
        id: commit,
        url: "https://github.com/athervvidhate/portfolio/commit/" + commit,
        author,
        date,
        time,
        timezone,
...
function displayStats() {
  ...
  const totCommits = dl.append("div");
  totCommits.append("dt").text("Total commits");
  totCommits.append("dd").text(commits.length);

  const totFiles = dl.append("div");
  totFiles.append("dt").text("Total Files");
  totFiles.append("dd").text(d3.group(data, (d) => d.file).size);
...
function createScatterplot() {
  ...
  // Create gridlines with colored ticks
  gridlines
    .call(d3.axisLeft(yScale).tickFormat("").tickSize(-usableArea.width))
    .selectAll(".tick line")
    .style("stroke", (d) => timeColorScale(d));
...
function updateTooltipPosition(event) {
  ...
  const x = Math.min(event.clientX + 10, maxX - 10);
  const y = Math.min(event.clientY + 10, maxY - 10);
